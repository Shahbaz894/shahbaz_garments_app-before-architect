...
//         BottomNavigationBarItem(
//           icon: IconButton(
//             icon: Icon(Icons.holiday_village, color: selectedItem.selectedItem == 4? Colors.green : Colors.grey), // Change the color here
//             // onPressed: () => selectedItem.setSelectedItem(1),
//             onPressed: (){
...
class MyBottomNavigationBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BottomNavigationBar(
      currentIndex: selectedItem.selectedItem,
      onTap: (index) {
        if (index >= 0 && index < 4) { // Ensure the index is within the valid range
          selectedItem.setSelectedItem(index);
        }
      },
      items: [
        BottomNavigationBarItem(
          icon: IconButton(
            icon: Icon(Icons.search, color: selectedItem.selectedItem == 0 ? Colors.green : Colors.grey),
            onPressed: () {
        ...
        BottomNavigationBarItem(
          icon: IconButton(
            icon: Icon(Icons.notification_add, color: selectedItem.selectedItem == 1 ? Colors.green : Colors.grey),
            onPressed: () {
              selectedItem.setSelectedItem(1);
              Navigator.push(context, MaterialPageRoute(builder: (context) => MyOrders()));
            },
        ...
        BottomNavigationBarItem(
          icon: IconButton(
            icon: Icon(Icons.favorite, color: selectedItem.selectedItem == 2 ? Colors.green : Colors.grey),
            onPressed: () {
              selectedItem.setSelectedItem(2);
              Navigator.push(context, MaterialPageRoute(builder: (context) => FavoriteScreen()));
            },
        ...
        BottomNavigationBarItem(
          icon: IconButton(
            icon: Icon(Icons.holiday_village, color: selectedItem.selectedItem == 3 ? Colors.green : Colors.grey),
            onPressed: () {
              selectedItem.setSelectedItem(3);
              Navigator.push(context, MaterialPageRoute(builder: (context) => DeliveryAddressScreen()));
            },
